...
"""
Choppediver OA solver + artifact logger.

What it does:
- Connects to the real WebSocket endpoint on :4443.
- Exploits the round loop by keeping heartbeats flowing so the server rarely
  reaches its recv() timeout-based deadline check.
- Classifies each PNG chart as buy / sell / hold.
- Stores a full JSONL event log, chart images, a human summary, and a zip
  archive for writeup evidence.
...
def classify_chart_png(png: bytes, threshold: float = 0.055) -> Prediction:
    ...
    if xs.size < 10:
        ...
        if xs_all.size < 10:
            return Prediction("hold", 0.0, 0.0, 0, 0, int(mask.sum()))
        mid = np.median(xs_all)
        left_y = float(np.median(ys_all[xs_all <= mid]))
        right_y = float(np.median(ys_all[xs_all > mid]))
        span_y = max(1, int(ys_all.max() - ys_all.min()))
        span_x = max(1, int(xs_all.max() - xs_all.min()))
    else:
        order = np.argsort(xs)
        xs, ys = xs[order], ys[order]
        n = xs.size
        k = max(5, n // 6)
        left_y = float(np.median(ys[:k]))
        right_y = float(np.median(ys[-k:]))
        span_y = max(1, int(np.percentile(ys, 95) - np.percentile(ys, 5)))
    ...
    if abs(score) < threshold:
        ...
        value = "sell"
    return Prediction(value, float(score), float(dy), int(span_x), int(span_y), int(mask.sum()))
        ...
        with connect(
            ...
            try:
                ...
                for i in range(1, total + 1):
                    ...
                    pred = classify_chart_png(png, threshold=args.threshold)
                    ...
                    logger.console(
                        ...
                        f"answer={pred.value} score={pred.score:+.4f} "
                        f"dy={pred.dy_pixels:+.1f}px span=({pred.span_x},{pred.span_y}) "
                        f"mask={pred.mask_pixels} classify={elapsed_ms:.2f}ms"
                    )
...
def main() -> None:
    ...
    ap.add_argument("--threshold", type=float, default=0.055, help="normalized slope threshold for HOLD")
    ap.add_argument("--open-timeout", type=float, default=10.0, help="WebSocket connect timeout")
    ap.add_argument("--artifact-dir", default="choppediver_artifacts", help="directory for logs, PNGs, summary, archive")
